...
HRESULT CTime::Init(void)
{
	...
	for (int nCntDigit = 0; nCntDigit < MAX_TIME_DIGIT; nCntDigit++)
	{
		m_apNumber[nCntDigit] = CNumber::Create(0,
			D3DXVECTOR3((float)(600 + nCntDigit * 40), 70.0f, 0.0f),
			D3DXVECTOR3(20, 20, 0),
...
void CTime::Update(void)
{
	...
	for (int nCntDigit = 0; nCntDigit < MAX_TIME_DIGIT; nCntDigit++)
	{
		...
		// ナンバーの配置
		m_apNumber[nCntDigit]->SetNumber((int)((m_nTime % (int)(powf(10.0f, (float)(MAX_TIME_DIGIT - nCntDigit)))) / (int)(powf((float)10, (float)(MAX_TIME_DIGIT - nCntDigit - 1)))));
	}
