...
//生成処理関数
//=============================================================================
Cbase_Cylinder * Cbase_Cylinder::Create(const D3DXVECTOR3 pos, const D3DXVECTOR3 size)
{
	// メモリの確保
	Cbase_Cylinder *pbase_Cylinder;
	pbase_Cylinder = new Cbase_Cylinder;
	// 引数の代入
	pbase_Cylinder->m_pos = pos;
	pbase_Cylinder->m_size = size;
	// 初期化
	pbase_Cylinder->Init();
	pbase_Cylinder->SetPos(pbase_Cylinder->m_pos);
	pbase_Cylinder->SetSize(pbase_Cylinder->m_size);
	// テクスチャ割り当て
	pbase_Cylinder->BindTexture(CResourceTexture::GetTexture(CResourceTexture::TEXTURE_PARTICLE_SKILL));
...
void Cbase_Cylinder::SetPos(const D3DXVECTOR3 pos)
{
	...
	{	// 頂点座標の設定
		...
		pVtx[2 * nCnt + 0].pos = D3DXVECTOR3(m_pos.x + sinf(theta) * (float)+(m_size.x / 2.0f) * 2.0  , m_pos.y + m_size.y / 2.0f, m_pos.z + cosf(theta) * ((float)m_size.z / 2.0f)* 2.0);
		pVtx[2 * nCnt + 1].pos = D3DXVECTOR3(m_pos.x + sinf(theta) * (float)+(m_size.x / 2.0f) * 0.75f, m_pos.y - m_size.y / 2.0f, m_pos.z + cosf(theta) * ((float)m_size.z / 2.0f)* 0.75f);
    }
...
void Cbase_Cylinder::SetColor(const D3DXCOLOR col)
{
	...
	for (int nCnt = 0; nCnt < NUM_CYL_VERTEX * 8; nCnt++)
	{	// 色の設定
		pVtx[nCnt].col = m_col;
	}
	// アンロック
